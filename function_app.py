...
@app.route(route="mock/forex/rate", methods=["GET"])
def mock_forex_rates(req: func.HttpRequest) -> func.HttpResponse:
    request_id = str(uuid.uuid4())
    interface_id = "INTSVC110_MOCKAPI"
    log_prefix = f"{interface_id} [{request_id}]"

    logging.info(f'{log_prefix}: Python HTTP trigger function received a request for mock forex rates.') 
    ...
    logging.info(f'{log_prefix}: Forex rates: {json.dumps(response)}') 
